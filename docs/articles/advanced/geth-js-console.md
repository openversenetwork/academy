...
## Run Openverse local node

- Clone the [openverse repository](https://github.com/openverse/openverse) (if you haven’t already)
...
Now we can use all implemented JSON-RPC methods. Find an exhaustive list of the supported JSON-RPC methods on [Openverse docs](https://docs.openverse.network/develop/api/ethereum-json-rpc/methods).
...
We get a big number because the result is denominated in `aopenverse`. We can convert to Openverse (10 ^18 `aopenverse`) using the `web3.fromWei` method:
...
We can perform token transfers using the corresponding method. For example, let's transfer 1 Openverse token from our account to another account:
...
The sender balance is reduced by 1 Openverse token and the fees paid for the transaction:
...
The receiver account balance initially was 100000000 Openverse tokens. After the transaction, the account balance has increased by 1 Openverse token.
