...
the [x/revenue module](https://docs.openverse.network/protocol/modules/revenue#concepts).
...
`openverse1keyy3teyq7t7kuxgeek3v65n0j27k20v2ugysf`):

```bash
# Register a revenue for your contract
openversed tx revenue register $CONTRACT $NONCE $WITHDRAWER \
--from=dev0 \ # contract deployer key
--gas=700000 --gas-prices=10000aopenverse \ # can vary depending on the network
```
...
```bash
# Check revenues
openversed q revenue contract $CONTRACT
```
...
[revenue parameter `DeveloperShares`](https://docs.openverse.network/protocol/modules/revenue#parameters),
which is controlled through governance.
You can query the parameters using our [OpenAPI documentation](https://api.openverse.dev/).

### Deployed Factory Pattern

You can also register a contract which has been deployed by a smart contract instead of an
[EOA](https://docs.openverse.network/protocol/modules/evm#accounts).
...
openversed tx revenue update $CONTRACT $WITHDRAWER \
--gas=700000 --gas-prices=10000aopenverse \
--from=mm
```
...
openversed tx revenue cancel $CONTRACT \
--gas=700000 --gas-prices=10000aopenverse \
--from=mm
```