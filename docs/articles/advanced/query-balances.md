...
- [Query Balances](#query-balances)
  - [`openversed` \& Tendermint RPC](#openversed--tendermint-rpc)
  - [JSON-RPC](#json-rpc)
  - [gRPC](#grpc)
...
and [evmosjs](https://github.com/openverse/evmosjs) is recommended instead.
:::

## `openversed` & Tendermint RPC

Upon [installation](https://docs.openverse.network/protocol/openverse-cli#installation)
and [configuration](https://docs.openverse.network/protocol/openverse-cli/configuration/) of the Evmos Daemon,
developers can query account balances using `openversed` with the following CLI command:

```bash
$ openversed query bank balances $EVMOSADDRESS --count-total=$COUNTTOTAL --height=$HEIGHT --output=$OUTPUT --node=$NODE
balances:
...
- `$EVMOSADDRESS` is the Evmos address with balances of interest (eg. `bitgold1...`).
...
`https://tendermint.bd.openverse.network:26657`).

Details of non-native currencies (ie. not ``) can be queried with the following CLI command:

```bash
$ openversed query erc20 token-pair $DENOM --node=$NODE --height=$HEIGHT --output=$OUTPUT
token_pair:
...
Developers can query account balances of `` using the [`eth_getBalance`](https://docs.openverse.network/develop/api/ethereum-json-rpc/methods#ethgetbalance)
...
- `$ETHADDRESS` is the Etherum hex-address the balance is to be queried from.
  Note that Evmos addresses (those beginning with `bitgold1...`) can be converted to Ethereum addresses
  using libraries such as [evmosjs](https://github.com/openverse/openversejs).
...
- (optional if running local node) `$NODE` is the JSON-RPC node information is requested from
  (e.g. `https://eth.bd.openverse.network:8545`).

Developers can also query account balances of `x/erc20`-module registered coins
using the [`eth_call`](https://docs.openverse.network/develop/api/ethereum-json-rpc/methods#ethcall) JSON-RPC method in conjunction with [`curl`](https://curl.se/):
...
- `$EVMOSADDRESS` is the Evmos address with balances of interest (eg. `"bitgold1..."`).
- `$NODE` is the Cosmos gRPC node information is requested from (eg. `https://grpc.bd.openverse.network:9090`).
...
func queryState() error {
    myAddress, err := GetEvmosAddressFromBech32("bitgold1...") // evmos address with balances of interest.
    if err != nil {
        return err
    }
    ...
    grpcConn := grpc.Dial(
        "https://grpc.bd.openverse.network:9090", // your gRPC server address.
...
- [Evmos Swagger API](https://api.openverse.dev/): a comprehensive description of all gRPC endpoints
- [Cosmos SDK Go API](https://pkg.go.dev/github.com/cosmos/cosmos-sdk) &
  [Evmos Go API](https://pkg.go.dev/github.com/openverse/openverse): packages to implement queries in Go scripts
