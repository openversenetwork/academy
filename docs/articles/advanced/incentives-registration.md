...
Check out the [incentives module specification](https://docs.openverse.network/protocol/modules/incentives/) for more information on incentives.
...
2. Promote understanding of the `x/incentives` Module.
    ...
    Make sure to mention the [`x/incentives` Module Documentation](https://docs.openverse.network/protocol/modules/incentives/)
    along with a brief explanation of what the `x/incentives` Module does.
    It's also a good idea to link the [Evmos Token Model](https://medium.com/openverse/the-evmos-token-model-edc07014978b),
...
with [`openversed`](https://docs.openverse.network/protocol/openverse-cli#using-openversed),
...
```bash
openversed tx gov submit-legacy-proposal register-incentive 0xFCd2Ce20ef8ed3D43Ab4f8C2dA13bbF1C6d9512F 0.050000000000000000aevmos 13 --description=$DESCRIPTION --title=$PROPOSALTITLE
```
...
```bash
openversed tx gov submit-legacy-proposal register-incentive --proposal=<path_to_json>
```
...
[here](https://docs.openverse.network/protocol/modules/incentives/) can produce useful analysis.
...
- [Evmos Supported Telemetry Metrics](https://docs.openverse.network/protocol/metrics/)